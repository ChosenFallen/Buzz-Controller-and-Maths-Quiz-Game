...
class buzz:
    def __init__(self):
        ...
        if self.device.is_kernel_driver_active(self.interface) is True:  # type: ignore
            self.kerneldriver = True
            self.device.detach_kernel_driver(self.interface)  # type: ignore
        else:
    ...
    def setlights(self, control):
        ...
        self.device.ctrl_transfer(0x21, 0x09, 0x0200, 0, [0x0, self.lights[0], self.lights[1], self.lights[2], self.lights[3], 0x0, 0x0])  # type: ignore
    ...
    def readcontroller(self, raw=False, timeout=None):
        ...
        try:
            cfg = self.device.get_active_configuration()  # type: ignore
            self.endpoint = cfg[(0, 0)][0]
            data = self.device.read(self.endpoint.bEndpointAddress, self.endpoint.wMaxPacketSize, timeout=timeout)  # type: ignore
            parsed = self.parsecontroller(data)
        except usb.core.USBError as e:
            if e[0] != 110:  # type: ignore
...
if __name__ == "__main__":
    buzz = buzz()  # type: ignore
    #    for x in range(16):
    # 	buzz.setlights(x)
    # 	time.sleep(1)
    buzz.setlights(1)  # type: ignore
    while True:
        r = buzz.readcontroller(timeout=500)  # type: ignore
        if r != None:
            print(bin(r))
